...
serve(async (req) => {
  ...
  try {
    ...
    const redirectUri = Deno.env.get('FIGMA_REDIRECT_URI');
    ...
    const callbackUrl = `${appOrigin}/integrations/callback?success=true&platform=figma`;
    ...
    const url = new URL(req.url);
    const appOrigin = Deno.env.get('APP_ORIGIN') || url.origin;
    const callbackUrl = `${appOrigin}/integrations/callback?error=${encodeURIComponent(errorMessage)}&platform=figma`;
